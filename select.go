...
// Errors can be retrieved through the methods on type [RecordSet].
func (s Store[R]) Select(ctx context.Context, db gsql.Handle, query string, args ...any) RecordSet[R] {
	// NOTE: This function body should be as short as possible to reduce the binary size after monomorphization.
	//       Any expression that does not depend on type R should be factored out into a reusable function.

	return RecordSet[R]{startSelectQuery(ctx, db, s.plan, query, args...)}
}
...
// Errors can be retrieved through the methods on type [RecordSet].
func (s Store[R]) SelectWhere(ctx context.Context, db gsql.Handle, partialQuery string, args ...any) RecordSet[R] {
	// NOTE: This function body should be as short as possible to reduce the binary size after monomorphization.
	//       Any expression that does not depend on type R should be factored out into a reusable function.

	return RecordSet[R]{startSelectWhereQuery(ctx, db, s.plan, partialQuery, args...)}
}

func startSelectQuery(ctx context.Context, db gsql.Handle, plan plan, query string, args ...any) recordSet {
	rows, err := db.GSQLQuery(ctx, query, args)
	if err != nil {
		return recordSet{Err: fmt.Errorf("during Query(): %w", err)}
	}

	columnNames, err := rows.Columns()
	if err != nil {
		err = fmt.Errorf("during rows.Columns(): %w", err)
		return recordSet{Err: errext.WithCleanup(err, "Rows.Close", rows.Close())}
	}
...
func startSelectWhereQuery(ctx context.Context, db gsql.Handle, plan plan, partialQuery string, args ...any) recordSet {
	if plan.Select.Query == "" {
		return recordSet{Err: errors.New("cannot execute SelectWhere() because query could not be autogenerated")}
	}
...
func (q PreparedSelectQuery[R]) Select(ctx context.Context, db gsql.Handle, args ...any) RecordSet[R] {
	// NOTE: This function body should be as short as possible to reduce the binary size after monomorphization.
	//       Any expression that does not depend on type R should be factored out into a reusable function.
	return RecordSet[R]{startSelectQuery(ctx, db, q.store.plan, q.query, args...)}
}
...
// RecordSet provides access to the result set from a [Store.Select], [Store.SelectWhere] or [PreparedSelectQuery.Select] call.
//
...
type RecordSet[R any] struct {
	recordSet
}

// recordSet contains the payload of [RecordSet].
...
func (s recordSet) collectRow(v reflect.Value, slots []any) error {
...
// This is the most versatile output format for type [RecordSet], but may cause a spike in memory usage for big result sets.
func (s RecordSet[R]) Collect() ([]R, error) {
	// NOTE: This function body should be as short as possible to reduce the binary size after monomorphization.
	//       Any expression that does not depend on type R should be factored out into a reusable function.
...
func (s RecordSet[R]) Foreach(action func(R) error) error {
	// NOTE: This function body should be as short as possible to reduce the binary size after monomorphization.
	//       Any expression that does not depend on type R should be factored out into a reusable function.
...
func (s RecordSet[R]) First() (R, error) {
	// NOTE: This function body should be as short as possible to reduce the binary size after monomorphization.
	//       Any expression that does not depend on type R should be factored out into a reusable function.
...
// FirstOrNone is like [RecordSet.First], but signals an empty result set using None instead of [sql.ErrNoRows].
func (s RecordSet[R]) FirstOrNone() (Option[R], error) {
	// NOTE: This function body should be as short as possible to reduce the binary size after monomorphization.
	//       Any expression that does not depend on type R should be factored out into a reusable function.
